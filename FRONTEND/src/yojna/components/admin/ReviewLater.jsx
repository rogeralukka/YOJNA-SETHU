    ...
    <div className="flex flex-col w-full p-4 sm:p-6 lg:p-margin-desktop gap-8 pb-24 max-w-7xl mx-auto animate-fade-in-up">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="font-body-lg text-xs sm:text-sm text-on-surface-variant dark:text-[#8A8F98] max-w-2xl">
        ...
        <div className="flex items-center gap-3 bg-surface-container dark:bg-[#0F1115] px-4 py-2.5 rounded-2xl border border-outline-variant/20 dark:border-white/[0.08]">
          <div className="flex flex-col">
            <span className="text-xl font-headline-md font-bold text-primary dark:text-primary-fixed">
              {reviewLaterApps.length}
            </span>
            <span className="text-[10px] font-label-bold text-on-surface-variant dark:text-[#8A8F98] uppercase tracking-wider">
              {t('savedItems')}
            </span>
          </div>
        </div>
      ...
      {reviewLaterApps.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-surface-container-low dark:bg-[#0F1115] rounded-3xl border border-outline-variant/20 dark:border-white/[0.08] text-center p-6">
          ...
          <p className="font-body-md text-xs text-on-surface-variant dark:text-[#8A8F98] max-w-sm mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {reviewLaterApps.map((app) => (
            <div
              ...
              className="relative flex flex-col bg-surface-container dark:bg-[#0F1115] rounded-2xl p-6 shadow-sm border border-outline-variant/30 dark:border-white/[0.08] hover:shadow-lg transition-all duration-300 hover:scale-[1.02] group"
            >
              {/* Dismiss from Review Later */}
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  onClick={() => setReviewLater(app.applicationId, false)}
                  className="p-1.5 rounded-full hover:bg-surface-container-high dark:hover:bg-[#16191F] text-on-surface-variant dark:text-[#8A8F98] hover:dark:text-[#EDEDED]"
                  title={t('remove')}
                >
              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-sm font-bold text-on-surface dark:text-[#EDEDED] truncate">
                    {app.userName}
                  </span>
              ...
              <div className="flex flex-col gap-3 flex-1 mb-5 text-xs">
                <div className="flex flex-col gap-0.5">
                  <span className="font-label-bold text-[10px] text-on-surface-variant dark:text-[#8A8F98] uppercase tracking-wider">
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  <div className="flex items-center gap-1.5 text-on-surface dark:text-[#8A8F98]">
                    <Icon name="calendar_today" size={15} className="text-outline" />
                    <span>{app.updatedAt || app.appliedAt}</span>
                  </div>
                </div>

                <div className="w-full h-[1px] bg-outline-variant/30 dark:bg-white/[0.08] my-1" />

                <div className="flex flex-col gap-1">
                  <div className="flex justify-between items-center text-[10px] font-label-bold text-on-surface-variant dark:text-[#8A8F98] uppercase tracking-wider">
                    <span>{t('auditStage')}</span>
                    <span>{t('status_pending')}</span>
                  </div>
                  <div className="w-full h-1.5 bg-surface-variant dark:bg-[#16191F] rounded-full overflow-hidden">
