...
export const DataProvider = ({ children }) => {
  ...
  const [currentView, setCurrentView] = useState(() => localStorage.getItem('yojanasetu_auth_role') === 'admin' ? 'admin-overview' : 'dashboard');
  ...
  // Connect to NagrikPath Household Context
  const householdContext = useHousehold ? useHousehold() : null;
  const activeMember = householdContext?.activeMember;

  // Reactively sync active member profile from HouseholdContext
  useEffect(() => {
    if (activeMember) {
      const age = activeMember.age || 20;
      const birthYear = new Date().getFullYear() - age;
      const dob = `${birthYear}-01-15`;

      let life_status = 'employed';
      let occupation = 'artisan_craftsperson';
      let sector = 'services';

      const occLower = (activeMember.occupation || '').toLowerCase();
      if (occLower.includes('farmer') || occLower.includes('agri')) {
        life_status = 'farmer';
        occupation = 'farmer';
        sector = 'agriculture';
        ...
        occupation = 'student';
        sector = 'education';
      } else if (occLower.includes('self-employed') || occLower.includes('business')) {
      ...
      setUserProfile(prev => ({
        ...prev,
        id: activeMember.memberId || prev.id,
        name: activeMember.name || prev.name,
        age: activeMember.age || prev.age,
        gender: activeMember.gender || prev.gender,
        category: activeMember.category || prev.category,
        income: activeMember.annualIncome !== undefined ? activeMember.annualIncome : prev.income,
        dob,
        life_status,
        occupation,
        sector,
        education_level: activeMember.occupation?.includes('Student') ? "Graduate / Bachelor's Degree" : prev.education_level,
        maskedAadhaar: activeMember.maskedAadhaar || prev.maskedAadhaar,
        maskedPan: activeMember.maskedPan || prev.maskedPan
      }));
    }
  }, [activeMember]);
